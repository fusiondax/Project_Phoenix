	...
	public static void convertTiledMapToGameMap(String tiledMapFileName, String gameMapFileName)
	{
		...
		for(MapLayer layer : layers)
		{
			for(MapObject mo : layer.getObjects())
			{
				...
				System.out.println("terrain: " + mo.getName()
				+ " X:" + mo.getProperties().get("x").toString() 
				+ " Y:" + mo.getProperties().get("y").toString());
				
				//TODO Fetch other custom propertied from TiledMap objects to populate entity components
		...
		for(JsonValue componentsJson : entityConfig.get("components"))
		{
			...
			switch(componentType)
			{
				case "Graphic":
				{
					...
					{
						((GraphicComponent)comp).texture = new Texture(componentsJson.get("texture").toString());
					}
				}
				case "Position":
				{
				...
				case "Terrain":
				{
					...
					if(componentsJson.get("type") != null)
					{
						...
						//TODO fetch terrain types from config file to entity component when loading the map
